...
for y_0 in y_inicial:
    y[0,:] = y_0 
    for i,t_i,t_f in zip(range(0,passos),t_in,t_fi):
        r = integrate.solve_ivp(pos, (t_i, t_f), y[i], args=[u[i]])
        y[i+1]=r.y[:,-1]  
    atrasos = 3
    atraso_0 = y[0:len(y)-atrasos,:]
    atraso_1 = y[1:len(y)-atrasos+1,:]
    atraso_2 = y[2:len(y)-atrasos+2,:]
    atraso_3 = y[3:len(y)-atrasos+3,:]
    x_nn = np.zeros([len(atraso_0),D_in])
    y_nn = np.zeros([len(atraso_0),D_out])
    
    for i,pos_0,pos_1,pos_2,pos_3 in zip(range(len(atraso_0)),atraso_0,atraso_1,atraso_2,atraso_3):
        x_nn[i,:]=[pos_1[0],pos_1[1],pos_2[0],pos_2[1],pos_3[0],pos_3[1]]
        y_nn[i,:]=[pos_0[0]]
...
for k,x_nn, y_nn in zip(range(R),x_saida[T+1:N],y_saida[T+1:N]):
        y_pred = model(x_nn/divisor)
        loss = loss_fn(y_pred,y_nn/divisor)
        print("TESTE -- perca: %.10f batelada: %i" %(float(loss),k+T))            
